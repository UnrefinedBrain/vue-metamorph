...
import { prettyPrint } from "../src/vendor/recast/main";
import {
  Type,
  builders as b,
  namedTypes as n,
  getBuilderName,
} from "../src/vendor/ast-types/main";
...
out.forEach(({ file, ast }) => {
  fs.writeFileSync(
    path.resolve(__dirname, `../src/vendor/ast-types/gen/${file}`),
    prettyPrint(ast, { tabWidth: 2, includeComments: true }).code
  );
});

function moduleWithBody(body: any[]) {
  // LOCAL DEVIATION: tag each generated export `@public`. vue-metamorph
  // re-exports these declarations from its entry point, so api-extractor wants
  // a release tag on every one; without them it reports ~300
  // ae-missing-release-tag warnings on each build, which buries anything real.
  body.forEach((node: any) => {
    if (node.type === "ExportNamedDeclaration") {
      node.comments = [b.commentBlock("* @public ", true, false)];
    }
  });
