...
// LOCAL DEVIATION: upstream calls `require("os")` lazily inside
// getLineTerminator so that browser bundles never pull in the module. That is a
// bare CommonJS `require`, which throws "require is not defined" as soon as
// these sources are loaded as real ESM. vue-metamorph is Node-only, so the
// import is hoisted to the top instead; the isBrowser() guard below is kept so
