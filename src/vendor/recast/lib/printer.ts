...
function genericPrintNoParens(path: any, options: any, print: any) {
  ...
  switch (n.type) {
    ...
    // Babel 8 encodes `abstract m(): void` as its own node type rather than
    // setting `abstract` on a MethodDefinition, so the keyword has to be added
    // here. The value is a TSEmptyBodyFunctionExpression, which printMethod
    // already handles: it has no body, so the member is terminated with `;`.
    case "TSAbstractMethodDefinition":
      return concat(["abstract ", printMethod(path, options, print)]);
    ...
    // The `value` of an abstract or declared method. Normally printed via
    // printMethod, which reaches through to these fields and supplies the
    // enclosing member's name; this case only matters if one is printed on
    // its own.
    ...
    case "PrivateIdentifier":
      // Babel 7 nests an Identifier under `id`; Babel 8's estree output puts
      // the bare name on `name`. Without the second branch `#field` prints
      // as `#`.
    ...
    case "ClassProperty":
    // As with TSAbstractMethodDefinition, Babel 8 puts the abstractness in the
    // node type; everything else prints like a PropertyDefinition.
    ...
    case "PropertyDefinition": {
      ...
      if (n.abstract || n.type === "TSAbstractPropertyDefinition") {
        parts.push("abstract ");
      }
    ...
    // Babel 8 gives template literal *types* their own node. It is shaped like
    // TemplateLiteral except the interpolations are types, held in `types`
    ...
    case "TSFunctionType":
      return concat([
        ...
        printFunctionParams(path, options, print),
        ") => ",
        printFirstOfNested(path, print, "typeAnnotation", "returnType", "typeAnnotation"),
      ]);
    ...
    case "TSMappedType": {
      // `readonly` and `optional` are `true`, `"+"` or `"-"`; the +/- forms are
      // modifier operators (`-readonly`, `-?`) and must keep their sign.
      if (n.readonly) {
        parts.push(n.readonly === "+" || n.readonly === "-" ? n.readonly : "", "readonly ");
      }

      // Babel 8 splits the old `typeParameter` (a TSTypeParameter carrying both)
      // into a `key` identifier plus a `constraint` type.
      if (n.key) {
        parts.push("[", path.call(print, "key"), " in ", path.call(print, "constraint"));
      } else {
        parts.push("[", path.call(print, "typeParameter"));
      }

      if (n.nameType) {
        parts.push(" as ", path.call(print, "nameType"));
      }

      parts.push("]");

      if (n.optional) {
        parts.push(n.optional === "+" || n.optional === "-" ? n.optional : "", "?");
      }
    ...
    case "TSTypeReference":
      return concat([
        ...
        printFirstOf(path, print, "typeArguments", "typeParameters"),
      ]);
    ...
    case "TSMethodSignature":
      ...
      parts.push(
        ...
        printFunctionParams(path, options, print),
        ")",
        printFirstOf(path, print, "returnType", "typeAnnotation"),
      );
    ...
    case "TSTypeParameter": {
      // Babel 8 exposes the variance and const modifiers: `<const T>`,
      // `<in out U>`.
      if (n["const"]) {
        parts.push("const ");
      }
      ...
      // `name` is an Identifier node under Babel 8 and a bare string under
      // Babel 7.
      parts.push(
        typeof n.name === "string" ? n.name : path.call(print, "name"),
      );
    ...
    case "TSEnumDeclaration": {
      ...
      // Babel 8 moved the members behind a TSEnumBody node.
      const memberLines = fromString(",\n").join(
        n.body ? path.map(print, "body", "members") : path.map(print, "members"),
      );
    ...
    // Only reached when a TSEnumBody is printed on its own; as part of a
    // TSEnumDeclaration the members are printed by the case above, which owns
    // the surrounding braces.
    case "TSEnumBody": {
      const bodyLines = fromString(",\n").join(path.map(print, "members"));
    ...
    // TSInterfaceHeritage and TSClassImplements are the Babel 8 replacements
    // for TSExpressionWithTypeArguments in `interface I extends J<K>` and
    // `class C implements D<E>` respectively. They print identically.
    ...
    case "TSImportType":
      // Babel 8 renamed `argument` to `source` and added `options`, which
      // carries import attributes: `import("m", { with: { type: "json" } })`.
      parts.push("import(", printFirstOf(path, print, "source", "argument"));
      ...
      parts.push(printFirstOf(path, print, "typeArguments", "typeParameters"));
...
// LOCAL ADDITION: @babel/parser 8 renamed a number of TypeScript AST fields
// (`typeParameters` -> `typeArguments` where they hold arguments rather than
// declarations, `typeAnnotation` -> `returnType` on function-shaped type nodes,
// `argument` -> `source` on TSImportType). Nodes built by ast-types' builders
// still use the Babel 7 names, so the printer has to accept either. This prints
// the first of `names` the node actually carries, and nothing if it has none.
function printFirstOf(path: any, print: any, ...names: string[]): any {
  const node = path.getValue();
...
// As above, but descends one level further (used to unwrap a TSTypeAnnotation).
function printFirstOfNested(
  path: any,
  print: any,
  nested: string,
  ...names: string[]
  ...
  for (const name of names) {
    if (node[name] != null) {
      return path.call(print, name, nested);
    }
  }
  return fromString("");
}
