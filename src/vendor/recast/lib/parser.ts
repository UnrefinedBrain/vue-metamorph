...
export function parse(source: string, options?: Partial<Options>) {
  ...
  // LOCAL DEVIATION: upstream falls back to Esprima's tokenizer when
  // NODE_ENV === "test", for the benefit of its own test suite. That branch is
  // reachable here — vitest sets NODE_ENV to "test" — but esprima is not a
  // dependency of this package, and the bare `require` would throw
  // "require is not defined" under ESM, hiding the actual problem. The
  // descriptive error is always the more useful outcome.
